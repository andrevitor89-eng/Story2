# -*- coding: utf-8 -*-
"""Montagem de video narrado com ffmpeg (Ken Burns ou clipes Kling + TTS)."""
...
@dataclass
class SceneClip:
    """Cena Ken Burns (imagem estatica + audio)."""
...
@dataclass
class AnimatedSceneClip:
    """Cena animada (clipe Kling + audio de narracao)."""
...
def probe_audio_duration_bytes(audio_bytes: bytes, *, ext: str = "mp3") -> float:
    """Duracao em segundos de um blob de audio."""
    with tempfile.TemporaryDirectory(prefix="probe_") as tmp:
        p = Path(tmp) / f"a.{ext.lstrip('.')}"
...
def mux_video_with_audio(
    ...
    *,
    ...
    """Substitui/anexa audio de narracao a um clipe de video (Kling)."""
    if not ffmpeg_available():
        raise AssembleError("ffmpeg nao encontrado no PATH")
    with tempfile.TemporaryDirectory(prefix="mux_") as tmp:
        root = Path(tmp)
        vid = root / "in.mp4"
        aud = root / "narr.mp3"
        out = root / "out.mp4"
        vid.write_bytes(video_bytes)
        aud.write_bytes(audio_bytes)
        audio_dur = _ffprobe_duration(aud) + 0.25
        # Escala e corta/estende o video ao comprimento do audio
        vf = (
            f"scale={width}:{height}:force_original_aspect_ratio=increase,"
            f"crop={width}:{height},fps=25,format=yuv420p"
        )
        _run(
            [
                "ffmpeg",
...
def concat_mp4_clips(clips: list[bytes], *, music_bytes: bytes | None = None) -> bytes:
    """Concatena MP4s (mesmos codecs) e opcionalmente mistura trilha."""
    if not clips:
        raise AssembleError("Nenhum clipe para concatenar")
    if not ffmpeg_available():
        raise AssembleError("ffmpeg nao encontrado no PATH")
...
def assemble_animated_narrated(
    scenes: list[AnimatedSceneClip],
    *,
    music_bytes: bytes | None = None,
    width: int = 720,
    height: int = 960,
) -> bytes:
    """Mux por cena (Kling+TTS) e concatena o video final."""
    if not scenes:
        raise AssembleError("Nenhuma cena para montar")
    muxed: list[bytes] = []
    for sc in scenes:
        if not sc.audio_bytes:
            raise AssembleError("Cena sem audio de narracao")
        muxed.append(
            mux_video_with_audio(sc.video_bytes, sc.audio_bytes, width=width, height=height)
        )
    return concat_mp4_clips(muxed, music_bytes=music_bytes)
...
def assemble_narrated_video(
    ...
    *,
    ...
    """Gera MP4 a partir de cenas (imagem + audio) via Ken Burns."""
    if not scenes:
        raise AssembleError("Nenhuma cena para montar")
    if not ffmpeg_available():
        raise AssembleError("ffmpeg nao encontrado no PATH")
    ...
    with tempfile.TemporaryDirectory(prefix="narrated_") as tmp:
        ...
        parts = [p.read_bytes() for p in clip_paths]
        return concat_mp4_clips(parts, music_bytes=music_bytes)
